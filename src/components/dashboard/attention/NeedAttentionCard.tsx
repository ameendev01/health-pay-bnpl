      ...
      <ul className="flex-1 min-h-0 flex flex-col gap-4 px-5 pb-0">
        {items.map(({ href, n, text, icon: Icon, tone }) => (
          <li key={text} className="min-h-0">
            <Link
  href={href}
  aria-label={`Review ${n} ${labelFor(text)}, ${toneLabel(tone)} priority`}
  className={clsx(
    ...
    "rounded-lg border border-neutral-200 bg-white px-0 py-2.5",
...
>
  ...
  >
    {tone === "danger" ? "High" : tone === "warning" ? "Medium" : "Info"}
  </span>
