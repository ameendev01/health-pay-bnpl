...
import POSWizard from './POSWizard';
...
const recentTransactions = [
  { id: 1, clinic: 'Sunrise Medical Center', patient: 'John D.', amount: '$450', status: 'completed', time: '2 min ago' },
  { id: 2, clinic: 'Valley Health Clinic', patient: 'Sarah M.', amount: '$1,200', status: 'processing', time: '5 min ago' },
  { id: 3, clinic: 'Metro Dental Care', patient: 'Michael R.', amount: '$850', status: 'completed', time: '8 min ago' },
  { id: 4, clinic: 'Family Health Partners', patient: 'Emma K.', amount: '$320', status: 'pending', time: '12 min ago' },
  { id: 5, clinic: 'Westside Cardiology', patient: 'David L.', amount: '$2,400', status: 'completed', time: '15 min ago' },
];
...
export default function Dashboard() {
  ...
  const handlePOSComplete = (planData: any) => {
    console.log('New payment plan created:', planData);
    // Here you would typically send the data to your API
    // For now, we'll just log it and show a success message
    setTimeout(() => {
      setIsPOSWizardOpen(false);
      alert(`Payment plan ${planData.id} has been created successfully for ${planData.patientName}!`);
    }, 1000);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="mt-2 text-gray-600">Overview of your healthcare payment platform</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">Latest payment activities across all clinics</p>
          </div>
          <div className="p-6">
            <div className="space-y-4">
              {recentTransactions.map((transaction) => (
                <div key={transaction.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center">
                        <span className="text-xs font-semibold text-white">
                          {transaction.clinic.split(' ').map(word => word[0]).join('').slice(0, 2)}
                        </span>
                      </div>
                      <div>
                        <p className="font-medium text-gray-900">{transaction.clinic}</p>
                        <p className="text-sm text-gray-600">Patient: {transaction.patient}</p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              >
                <span className="font-medium text-gray-900 group-hover:text-teal-700">Add New Clinic</span>
              ...
              <button 
                ...
                className="w-full flex items-center justify-between p-3 text-left border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-teal-300 transition-all duration-200 group"
              >
                <span className="font-medium text-gray-900 group-hover:text-teal-700">Create Payment Plan</span>
              ...
              <button className="w-full flex items-center justify-between p-3 text-left border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-teal-300 transition-all duration-200 group">
                <span className="font-medium text-gray-900 group-hover:text-teal-700">View Reports</span>
