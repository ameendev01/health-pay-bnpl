...
const STATUS_STYLES: Record<ClaimStatus, { pill: string; icon: string }> = {
  ...
  processing: {
    pill: "bg-amber-500/10 border-amber-500/30 border-[1.5px] text-amber-700",
    icon: "text-amber-600",
  },
  paid: {
  ...
  rejected: {
    pill: "bg-red-500/10 border-red-500/30 border-[1.5px] text-red-700",
    icon: "text-red-600",
  },
};
...
const getAgingDays = (claim: Claim) => {
  const submission = claim.submissionDate
    ? new Date(claim.submissionDate)
    : new Date(claim.createdAt);
  const now = new Date();
  return Math.max(
    0,
    Math.floor((now.getTime() - submission.getTime()) / (1000 * 60 * 60 * 24))
  );
};
...
export default function ClaimsTable({
  ...
  const payers = useMemo(
    () =>
      Array.from(
        new Set(claims.map((c) => c.payerName).filter(Boolean))
      ).sort(),
    [claims]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return claims.filter((c) => {
      ...
      const matchesClinic =
        clinic === "all" || (c.clinicId ?? "N/A") === clinic;
  ...
  const grouped = useMemo(() => {
    const by: Record<ClaimStatus, Claim[]> = {
      pending: [],
      processing: [],
      paid: [],
      denied: [],
    ...
    >
                ...
                >
                  ...
                  <div className="px-4 max-h-[70vh] overflow-auto">
                    ...
                    <Section title="Payer" defaultOpen>
                      <Select
                        value={payer}
                        onValueChange={(v: any) => setPayer(v)}
                      >
                    ...
                    <Section title="Clinic" defaultOpen>
                      <Select
                        value={clinic}
                        onValueChange={(v: any) => setClinic(v)}
                      >
                        <SelectTrigger className="h-9 w-full">
              ...
              <AppliedChips
                entries={
                  [
                    status !== "all" && {
                      key: "status",
                      label: `Status: ${titleCase(String(status))}`,
                    },
                    payer !== "all" && {
                      key: "payer",
                      label: `Payer: ${payer}`,
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-white grid grid-cols-[24px_minmax(220px,1.2fr)_1fr_.9fr_.9fr_.8fr_.8fr_.7fr_.8fr_48px] gap-3 px-4 pt-3 pb-2 text-[12px] font-semibold text-gray-500 border-t border-[#ece9dd]">
          ...
          <HeaderButton
            label="Claim"
            active={sort.key === "claim"}
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "claim")}
          />
          <HeaderButton
            label="Patient"
            active={sort.key === "patient"}
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "patient")}
          />
          <HeaderButton
            label="Clinic"
            active={sort.key === "clinic"}
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "clinic")}
          />
          <HeaderButton
            label="Payer"
            active={sort.key === "payer"}
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "payer")}
          />
          <HeaderButton
            label="Amount"
          ...
          <HeaderButton
            ...
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "status")}
          />
          <HeaderButton
            label="Aging"
            active={sort.key === "aging"}
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "aging")}
          />
          <HeaderButton
            label="Updated"
            active={sort.key === "updated"}
            dir={sort.dir}
            onClick={() => toggleSort(setSort, "updated")}
          />
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  const selIds = Object.keys(selected).filter(
                    (id) => selected[id]
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[13px] text-gray-900">
                        ...
                        {c.paidAmount ? (
                          <span className="ml-2 text-[12px] text-emerald-600">
                ...
                <div className="px-4 pb-4">
                  <Button
                    ...
                    onClick={() =>
                      setVisibleByGroup((s) => ({
                        ...s,
                        [g]: Math.min(list.length, s[g] + STEP),
                      }))
                    }
                  >
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() =>
              setVisibleByGroup((s) => {
                const next = { ...s };
                (Object.keys(grouped) as (keyof typeof grouped)[]).forEach(
                  (k) => (next[k] = grouped[k].length)
                );
                return next;
              })
            }
...
function toggleSort(
  set: React.Dispatch<React.SetStateAction<SortState>>,
  key: SortKey
) {
  set((s) =>
    s.key === key
