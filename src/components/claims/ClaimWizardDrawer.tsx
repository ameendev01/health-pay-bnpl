...
import { StepPatient, StepCoverage, StepServices, StepAttachments, StepReview } from "./ClaimWizardSteps";
...
export default function ClaimWizardDrawer({
  ...
  // --- core wizard state
  const [activeTab, setActiveTab] = useState<"patient" | "coverage" | "services" | "attachments" | "review">("patient");
  ...
  // keep derived fields synced on-draft
  useEffect(() => {
    setDraft((d) => ({
      ...d,
  ...
  async function handleSubmit() {
    ...
    if (errs.length) {
      // smart jump to the tab that can resolve the first issue
      const first = errs[0];
      if (first.field?.startsWith("patientId") || first.field?.includes("dateOfService")) setActiveTab("patient");
      else if (first.field?.includes("payer") || first.field?.includes("member")) setActiveTab("coverage");
      else if (first.field?.startsWith("lines") || first.field?.startsWith("diagnosis")) setActiveTab("services");
      else if (first.field?.startsWith("attachments")) setActiveTab("attachments");
        ...
        <SheetHeader className="pb-6">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-200">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold text-gray-900">{formatCurrency(gross)}</span>
            </div>
            <div className="flex items-center space-x-2">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
          <TabsList className="grid w-full grid-cols-5 bg-gray-100 border-2 border-gray-200 pb-8">
            {STEPS.map(({ key, title, valid, icon: Icon }) => (
              <TabsTrigger key={key} value={key} className="flex items-center space-x-1">
                ...
                {!valid && <span className="ml-1 inline-block h-1.5 w-1.5 rounded-full bg-amber-500" />}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          <div className="mt-6">
            ...
            <TabsContent value="services">
              <div className="rounded-xl border border-gray-200 bg-white p-5">
                <StepServices draft={draft} set={set} derived={{ gross, payer, patient, financed }} />
              </div>
            </TabsContent>
            ...
            <TabsContent value="review">
              <div className="rounded-xl border border-gray-200 bg-white p-5">
                ...
                <StepReview draft={draft} derived={{ gross, payer, patient, financed }} onSubmit={handleSubmit} />
              </div>
            </TabsContent>
          </div>
        </Tabs>

        {/* Sticky footer for wizard nav (kept subtle; ClaimDrawer doesn’t use one, but wizard needs it) */}
        <div className="sticky bottom-0 mt-6 border-t border-gray-200 bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/75">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" className="h-8" onClick={() => goto(-1)} disabled={stepIndex === 0}>
                Back
              </Button>
              {stepIndex < STEPS.length - 1 ? (
                <Button className="h-8" onClick={() => goto(1)}>
                  Next <ChevronRight className="h-4 w-4 ml-1" />
                </Button>
              ) : (
                <Button className="h-8" onClick={handleSubmit} disabled={submitting || issues.length > 0}>
